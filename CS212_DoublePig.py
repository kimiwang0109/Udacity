...
def Q_pig_d(state, action, Pwin_d): 
    global n
    n+=1
    print state, action, n
    if action == 'hold':
        return Pwin_d(do('hold',state,dierolls()))
    if action == 'roll':
        return (1- Pwin_d(do('roll',state, iter([1])))+sum(Pwin_d(do('roll', state, iter([x])) for x in range(2,7))))/6 
        #return 1-Pwin_d(do('roll',state,dierolls()))
    if action == 'double':
        return 0.5
    if action == 'decline':
        return 0
    if action == 'accept':
        return 1-Pwin_d(do('accept', state, dierolls()))
...
def Pwin3(me, you, pending):
   ## your code here
    if me + pending >= goal:
        ...
        gotta_roll = (1 - Pwin3(you, me + 1, 0) + sum(Pwin3(me, you, pending + d) for d in (2,3,4,5,6))) / 6.
        return gotta_roll if (pending == 0) else max(gotta_roll,1 - Pwin3(you, me + pending, 0))
