...
def boggle_words(board, minlength=3):
    ...
    # your code here  
    result = []  
    def findBoggle( curStr, path):
        if curStr in WORDS and len(curStr) >= minlength:
            result.append(curStr)
        if curStr in PREFIXES:
            for j in neighbors(path[-1], size(board)):
                if j not in path and board[j]!= '|':
                    findBoggle(curStr+board[j], path+[j])
...
print boggle_words(Board('ABC ABC ABC'))
